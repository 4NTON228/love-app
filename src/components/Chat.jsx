...
export default function Chat({ session, profile, darkMode }) {
  ...
  async function startVideoCircle() {
    try {
      ...
      if (previewVideoRef.current) { previewVideoRef.current.srcObject = stream; previewVideoRef.current.play() }
      ...
      recorder.onstop = async () => {
        ...
        try {
          ...
          await supabase.from('messages').insert({ user_id: session.user.id, video_url: videoUrl, is_video_circle: true })
          scrollToBottom()
        } catch (err) { console.error(err) }
  ...
  function stopVideoCircle() { if (mediaRecorderRef.current?.state === 'recording') mediaRecorderRef.current.stop() }
